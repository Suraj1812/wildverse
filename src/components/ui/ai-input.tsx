    ...
    <div className="flex items-center justify-center" style={{ width: FORM_WIDTH, height: FORM_HEIGHT }}>
      ...
      >
        <FormContext.Provider value={ctx}>
          <DockBar />
          <InputForm inputRef={textareaRef} onSuccess={handleSuccess} onSubmit={onSubmit} />
...
function DockBar() {
  const { showForm, successFlag, triggerOpen } = useFormContext();
  return (
          ...
          <AnimatePresence mode="wait">
            {showForm ? (
              ...
              >
                <ColorOrb
                  dimension="24px"
                  tones={successFlag ? { base: "oklch(75% 0.15 150)" } : { base: "oklch(22.64% 0 0)" }}
                />
              </motion.div>
            )}
          </AnimatePresence>
...
function InputForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError("Request failed. Check your Gemini API key and try again.");
    } finally {
      setIsLoading(false);
    }
  }
        ...
        {showForm && (
          ...
          >
            <div className="flex justify-between py-1">
              ...
              >
                <KeyHint>{isLoading ? "..." : "⌘"}</KeyHint>
                <KeyHint className="w-fit">{isLoading ? "Sending" : "Enter"}</KeyHint>
              </button>
            </div>
            <textarea
              ref={inputRef}
              ...
              name="message"
              className="h-full w-full resize-none rounded-md p-4 outline-0"
              required
