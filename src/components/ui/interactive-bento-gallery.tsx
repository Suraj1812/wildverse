...
const MediaItem = ({
  ...
  useEffect(() => {
    ...
    const handleVideoPlay = async () => {
      if (!currentVideo || !isInView || !mounted) return;
  ...
  return (
    <div className={`${className} relative cursor-pointer overflow-hidden`} onClick={onClick}>
      <Image src={item.url} alt={item.title} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover" />
    </div>
  );
};
  ...
  return (
    <>
      <motion.div
        ...
        className="fixed inset-0 z-10 w-full min-h-screen overflow-hidden rounded-none backdrop-blur-lg sm:h-[90vh] sm:rounded-lg md:h-[600px] md:rounded-xl"
      >
        ...
        <motion.button
          className="absolute top-2 right-2 rounded-full bg-gray-200/80 p-2 text-xs text-gray-700 backdrop-blur-sm hover:bg-gray-300/80 sm:top-2.5 sm:right-2.5 sm:text-sm md:top-3 md:right-3"
          onClick={onClose}
